...
import { IconSettings, IconLoader, IconCheck, IconTrash, IconPlus, IconShoppingCart, IconPackage, IconZap } from '../components/Icons';
...
interface Product {
    ...
    category: string;
    price: number;
    include_in_product_rateio: boolean;
}
...
export const PdvConfigPage: React.FC = () => {
    ...
    const fetchData = async () => {
        ...
        try {
            const [pts, prods, vis] = await Promise.all([
                ...
                supabase.from('products').select('id, name, category, price, include_in_product_rateio').order('name'),
    ...
    const handleToggleRateio = async (e: React.MouseEvent, prodId: number, current: boolean) => {
        e.stopPropagation();
        setSaving(true);
        try {
            const { error } = await supabase
                .from('products')
                .update({ include_in_product_rateio: !current })
                .eq('id', prodId);
            
            if (error) throw error;
            
            setProducts(prev => prev.map(p => p.id === prodId ? { ...p, include_in_product_rateio: !current } : p));
        } catch (err: any) {
            alert('Erro ao atualizar participação: ' + err.message);
        } finally {
            setSaving(false);
        }
    };
                        ...
                        <div className="bg-white rounded-[2.5rem] w-full max-w-4xl h-[85vh] flex flex-col overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200 relative z-10">
                            <header className="p-8 border-b border-gray-100 flex justify-between items-center bg-gray-50">
                                <div>
                                    ...
                                    <p className="text-gray-500 text-sm font-medium">Selecione os produtos do PDV e use o ícone de raio <IconZap className="w-3 h-3 inline text-amber-500" /> para marcar itens de produção (10% rateio).</p>
                                </div>
                                                ...
                                                return (
                                                    <div key={prod.id} className="relative group">
                                                        <button
                                                            ...
                                                            className={`w-full p-4 rounded-2xl border-2 text-left transition-all relative ${isVisible
                                                        ...
                                                        {/* Toggle para Rateio de Produção */}
                                                        <button
                                                            onClick={(e) => handleToggleRateio(e, prod.id, prod.include_in_product_rateio)}
                                                            className={`absolute top-3 right-3 p-2 rounded-xl transition-all shadow-sm z-10 ${
                                                                prod.include_in_product_rateio 
                                                                ? 'bg-amber-100 text-amber-600 border border-amber-200' 
                                                                : 'bg-white text-gray-300 border border-gray-100 hover:text-amber-500'
                                                            }`}
                                                            title={prod.include_in_product_rateio ? "Participa do Rateio (10%)" : "Ativar Rateio de Produção"}
                                                        >
